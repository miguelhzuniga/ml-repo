...
#     K = confusion_matrix(numpy.asarray(y), numpy.asarray(y_pred))

#     # Cálculo de métricas por clase
#     TP = numpy.diag(K)
#     FN = numpy.sum(K, axis=1) - TP
#     FP = numpy.sum(K, axis=0) - TP
#     TN = numpy.sum(K) - (TP + FP + FN)

#     epsilon = 1e-10
#     sensibility = TP / (TP + FN + epsilon)  # Recall por clase
#     specificity = TN / (TN + FP + epsilon)
#     accuracy = numpy.sum(TP) / numpy.sum(K)  # Accuracy general
...
def Confusion(m, X, y):
    ...
    # Ensure y_pred is in label form (not one-hot encoded)
    if y_pred.ndim > 1:  
        y_pred = numpy.argmax(y_pred, axis=1)
    if y.ndim > 1:  
        y = numpy.argmax(y, axis=1)
    
    # Flatten arrays to ensure they have the same dimensions
    y = numpy.ravel(y)
    y_pred = numpy.ravel(y_pred)
    
    # Number of classes
    num_classes = max(numpy.max(y), numpy.max(y_pred)) + 1
    
    # Compute confusion matrix manually
    K = numpy.zeros((num_classes, num_classes), dtype=int)
    for true_label, pred_label in zip(y, y_pred):
        K[true_label, pred_label] += 1
    ...
    sensibility = TP / (TP + FN + epsilon)  # Recall per class
    specificity = TN / (TN + FP + epsilon)
    accuracy = numpy.sum(TP) / numpy.sum(K)  # Overall accuracy
    F1 = 2 * TP / (2 * TP + FP + FN + epsilon)
