...
class Adam( GradientDescent ):
  ...
  def _fit( self, X_tr, y_tr, X_te, y_te, batches ):
    ...
    cb1 = float( 1 ) - b1
    cb2 = float( 1 ) - b2
    mt = numpy.zeros( ( 1, self.m_Model.size( ) ) )
    vt = numpy.zeros( ( 1, self.m_Model.size( ) ) )
    ...
    while not stop:
      ...
      if not math.isnan( J_tr ) and not math.isinf( J_tr ):
        ...
        if not self.m_Debug is None:
          # print(G.shape)
          # stop = self.m_Debug( t, ( G.T @ G )[ 0 , 0 ] ** 0.5, J_tr, J_te ) # Old
          stop = self.m_Debug( t, ( G @ G.T )[ 0 , 0 ] ** 0.5, J_tr, J_te ) # New
        # end if
